...
Integration is a one-time setup (\texttt{ob init}, \texttt{ob author add}, \texttt{ob register add}), after which the pipeline calls the tracking API once per record. Internally, tracking computes the SHA-256 hash, associates each record with its registered sections, checks for duplicates, and writes to process-ID-isolated files (\texttt{docidx.\{pid\}}), which \texttt{ob clean} later merges into sharded buckets; \texttt{ob} is a non-invasive observer of the pipeline.

We validate framework-agnostic integration with two pipelines. For Datatrove~\cite{penedo2024datatrove}, an \texttt{OBTrack} PipelineStep reads contributor metadata from \texttt{Document.metadata} and writes token-index entries. For HuggingFace Datasets~\cite{lhoest2021datasets}, an \texttt{OBMapper} callable registers authors and sections and writes document-index entries per record. Both adapters produce identical \texttt{.ob/} directories; experimental overhead is reported in Section~\ref{sec:evaluation}.
